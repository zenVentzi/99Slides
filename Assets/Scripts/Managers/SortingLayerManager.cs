...
namespace Assets.Scripts.Managers
{
    public class SortingLayerManager : MonoBehaviour
    {
        ...
        public static bool IsTopmost(GameObject go, bool belowMouse = true)
        {
            if (go == null)
            {
                throw new Exception("go, IsTopMost=null");
            }

            Vector3 wp = belowMouse ? 
            ...
            var topMostSoFar = hits[0].collider.gameObject;
        ...
        public static GameObject GetTopmostBelowMouse()
        {
            ...
            {
                if (IsTopmost(allHits[i].collider.gameObject))
                {
